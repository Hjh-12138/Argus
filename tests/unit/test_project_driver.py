...
def _snapshot() -> SnapshotReference:
    return SnapshotReference(
        snapshot_id="snap-1", source_root="/workspace/snapshot",
        files=[{"path": "app.py", "sha256": "0" * 64, "size": 10, "language": "py"}],
        archive_sha256="1" * 64)
...
class ProjectDriverTest(unittest.TestCase):
    ...
    def test_run_orders_assessors_meta_synth(self):
        ...
        outcome = _driver(client, self.tmp).run({"project_id": "proj-1"}, _snapshot())
    ...
    def test_required_assessor_failure_gives_human_wait(self):
        ...
        outcome = _driver(client, self.tmp).run({"project_id": "proj-2"}, _snapshot())
    ...
    def test_missing_artifact_never_passes(self):
        ...
        client = NoArtifact()
        outcome = _driver(client, self.tmp).run({"project_id": "proj-3"}, _snapshot())
        self.assertEqual(outcome.status, "human-wait")
    ...
    def test_register_task_uses_observed_skill_identity(self):
        ...
        driver._register_task(
            "proj-x", "proj-x-assessor-dep", "argus-dep", "assess",
            "argus-dependency-inspect", _snapshot(), {})
